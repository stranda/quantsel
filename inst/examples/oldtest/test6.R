...
tops <-   floor(seq(0,10000,length=6))
locs=NULL
for (i in 1:(length(tops)-1))
{
    locs <- rbind(locs,
                  data.frame(lft=c(0,501,rights[c(-1:-2)]-diff(rights[-1])+1),
                             bot=rep(tops[i+1]-(tops[2]-tops[1]),50),
                             rgt=rights,
                             top=tops[i+1]))
}

lfts=which(locs$lft==0)
k=(1.50 * (sqrt((locs[,3]-locs[,1])*(locs[,4]-locs[,2]))))
